import React from 'react';

interface BadgesInterface {
    is_event?: boolean,
    is_all_free: boolean,
    is_waitfree: boolean,
    is_waitfree_plus: boolean,
    imgStyle?: object,
}
export const Badges = (props:BadgesInterface) => {
    let imageSources:string[] = [];
    if (props.is_event) {
        imageSources.push("data:image/svg+xml;base64,")
    } else if (props.is_waitfree_plus) {
        imageSources.push("data:image/svg+xml;base64,")
    } else if (props.is_waitfree) {
        imageSources.push("data:image/svg+xml;base64,")
    }
    if (props.is_all_free) {
        imageSources.push("data:image/svg+xml;base64,")
    }
    
    // console.log(imageSources)
    return (
        <>
        {
            false
            ? null
            : <div style={props.imgStyle}>
            {
                imageSources.map((badgeSrc:string, index:number) => {
                    return (
                        <img src={badgeSrc} key={index}
                        style={{
                            zIndex: '100',
                            position: 'relative'
                        }}
                        />
                    )
                })
            }
            </div>
        }
        </>
    )
}